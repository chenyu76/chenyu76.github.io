/* =========================================================================
 * Blockwise Direct Search (BDS) demo - Algorithm 1 of
 * "Blockwise Direct Search Methods" (Huang, Li, Zhang 2026, see the PhD
 * symposium slides). In this 2-D demo the two "blocks" are the coordinate
 * axes, and - unlike plain direct search - each block keeps its OWN step
 * size alpha_x / alpha_y:
 *
 *   for k = 0, 1, ... do
 *     pick block i (here x and y alternately, so both are visited forever)
 *     poll d in {+e_i, -e_i}: xk + alpha_i d
 *     if some poll improves f(xk) - c*alpha_i^2:
 *         accept it and  alpha_i <- gamma*alpha_i   (gamma = 2)
 *     else:
 *         stay at xk and alpha_i <- theta*alpha_i   (theta = 1/2)
 *     the other block's step size is unchanged
 *
 * Animation language is identical to the other previews (shared heatmap,
 * black markers/trail, no step-size circles).
 * ====================================================================== */
(function() {
'use strict';
...
/* ---- BDS iteration (records the film for playback) --------------------- */
function searchLoop(land, x0, cfg) {
  ...
  const maxIter = P.maxIter !== undefined ? P.maxIter : 320;

  let x = {x : x0.x, y : x0.y};
  let fx = land.f(x.x, x.y);
  let alphaX = alpha0, alphaY = alpha0; // one step size per block
  const history = [];
  let reason = 'alpha';

  for (let k = 0; k < maxIter; k++) {
    const axis = k % 2; // alternate the x block and the y block
    const alpha = axis === 0 ? alphaX : alphaY;
    const probes = [
      axis === 0
        ? {p : {x : clamp01(x.x + alpha), y : x.y}}
        : {p : {x : x.x, y : clamp01(x.y + alpha)}},
      axis === 0
        ? {p : {x : clamp01(x.x - alpha), y : x.y}}
        : {p : {x : x.x, y : clamp01(x.y - alpha)}},
    ];
    probes[0].v = land.f(probes[0].p.x, probes[0].p.y);
    probes[1].v = land.f(probes[1].p.x, probes[1].p.y);
    let best = probes[0].v < probes[1].v ? 0 : 1;
    const improved = probes[best].v < fx; // margin c*alpha^2 omitted (c = 0)
    ...
    history.push({
      ...
      fx : fx,
      alpha : alpha,
      alphaX : axis === 0 && improved ? alphaX * expand : axis === 0 ? alphaX * shrink : alphaX,
      alphaY : axis === 1 && improved ? alphaY * expand : axis === 1 ? alphaY * shrink : alphaY,
      probes : probes,
      best : best,
      accept : improved,
    });
    ...
    if (improved) {
      x = probes[best].p;
      fx = probes[best].v;
      if (axis === 0) alphaX = alphaX * expand; else alphaY = alphaY * expand;
    } else {
      if (axis === 0) alphaX = alphaX * shrink; else alphaY = alphaY * shrink;
      // stop only once both blocks are below the requested precision
      if (alphaX <= alphaTol && alphaY <= alphaTol) { reason = 'alpha'; break; }
    }
  }
  return {history : history, xFinal : x, fxFinal : fx, reason : reason, alphaFinal : [alphaX, alphaY]};
}
...
function draw(now) {
  ...
  // polled points of the current block (revealed one by one)
...
/* ---- playback -------------------------------------------------------------- */
const SETTINGS = {alpha0 : 0.09, alphaTol : 1e-2, maxIter : 320};
const T = {
  probe : 180,
  cand : 320,
  move : 300,
...
async function playStep(s) {
  ...
  state.move = null;
  state.rejectFlash = null;
  // reveal the two polled points of this block
  for (let i = 0; i < s.probes.length; i++) {
    state.probeShow = i + 1;
    await sleep(T.probe);
  }
  await sleep(T.cand * 0.4);
  // decision
  ...
  if (s.accept) {
    ...
    // no improving poll: flash the best poll and shrink this block's step
    state.rejectFlash = {
